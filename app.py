...
def compact_bar_chart(frame, x_col, y_col, color="#FA6560", height=240):
    return (
        alt.Chart(frame)
        .mark_bar(
        ...
        .encode(
            x=alt.X(x_col, title=None, sort="-y"),
...
def compact_bar_chart_by_category(frame, x_col, y_col, height=240):
    return (
        alt.Chart(frame)
...
def horizontal_bar_chart(frame, x_col, y_col, color="#2563EB", category_col=None, height=240):
    ...
    # 👉 Mode 1: pakai kategori (Dropout vs Graduate)
    if category_col:
        chart = chart.encode(
            color=alt.Color(
                f"{category_col}:N",
...
with dashboard_tab:
    ...
    with top_board[1].container(border=True, height="stretch"):
        ...
        st.altair_chart(
            compact_bar_chart_by_category(status_counts, "Student status:N", "Students:Q", height=320),
            width="stretch",
        )
    ...
    with row_one[0].container(border=True, height="stretch"):
        ...
        st.altair_chart(
            stacked_bar_chart(fee_chart, "Tuition status", ["#EB2525", "#2563EB", "#BFDBFE"], height=240),
            width="stretch",
        )
    ...
    with row_one[1].container(border=True, height="stretch"):
        ...
        st.altair_chart(
            horizontal_bar_chart(at_risk_course, "Dropout cases:Q", "Course_name:N"),
            width="stretch",
        )
    ...
    with row_one[2].container(border=True, height="stretch"):
        ...
        st.altair_chart(
            compact_bar_chart(age_dropout, "Age_band:N", "Dropout rate:Q", height=240),
            width="stretch",
        )
...
with prediction_tab:
    ...
    if submitted:
        ...
        with result_cols[1].container(border=True, height="stretch"):
            ...
            st.altair_chart(
                horizontal_bar_chart(proba_df, "Probability:Q", "Status:N", category_col="Status", height=220),
                width="stretch",
            )
        ...
        with sim_cols[0].container(border=True, height="stretch"):
            ...
            def highlight_dropout(val):
                if val == "Dropout":
                    return "background-color: #ef4444; color: white;"  # merah kuat
                elif val == "Graduate":
                    return "background-color: #22c55e; color: white;"  # hijau
                return ""

            styled_df = scenario_df.style.map(highlight_dropout, subset=["Predicted status"]).background_gradient(
                    subset=["Dropout probability"],
                    cmap="Reds"  # gradasi merah
                )
